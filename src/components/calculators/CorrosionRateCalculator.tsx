...
export default function CorrosionRateCalculator() {
  ...
  const calculateSimpleRate = () => {
    ...
    if (corrosionRate < 0.1) {
      recommendation = 'Низкая скорость коррозии (< 0.1 мм/год). Срок следующего освидетельствования - 4 года (РД 03-421-01, п. 5.4).';
    } else if (corrosionRate < 0.2) {
      recommendation = 'Умеренная скорость коррозии (0.1-0.2 мм/год). Срок следующего освидетельствования - 2 года (РД 03-421-01, п. 5.4).';
    } else if (corrosionRate < 0.5) {
      recommendation = 'Повышенная скорость коррозии (0.2-0.5 мм/год). Необходим ежегодный контроль толщины стенки (РД 03-421-01, п. 5.4).';
    } else {
      recommendation = 'ВНИМАНИЕ! Высокая скорость коррозии (> 0.5 мм/год). Требуется немедленная оценка состояния и возможная замена элемента (РД 03-421-01, п. 5.5).';
    }
  ...
  const calculateAdvancedRate = () => {
    ...
    let recommendation = '';
    if (trend === 'increasing') {
      recommendation = 'ВНИМАНИЕ! Скорость коррозии увеличивается. Необходимо выявить причину ускорения процесса и принять меры защиты (РД 03-421-01, п. 5.5).';
    } else if (trend === 'decreasing') {
      recommendation = 'Скорость коррозии снижается. Возможно, защитные меры эффективны. Продолжайте мониторинг согласно РД 03-421-01.';
    } else {
      if (currentRate < 0.1) {
        recommendation = 'Низкая стабильная скорость коррозии (< 0.1 мм/год). Срок следующего освидетельствования - 4 года (РД 03-421-01, п. 5.4).';
      } else if (currentRate < 0.2) {
        recommendation = 'Умеренная стабильная скорость коррозии (0.1-0.2 мм/год). Срок следующего освидетельствования - 2 года (РД 03-421-01, п. 5.4).';
      } else {
        recommendation = 'Повышенная стабильная скорость коррозии (> 0.2 мм/год). Необходим ежегодный контроль и оценка необходимости защитных мер (РД 03-421-01, п. 5.4).';
      }
    }
